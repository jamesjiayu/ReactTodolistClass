    ...
    return (
      <div className="todolist">
        ...
        <div className="todolist-form">
          <input className="todolist-input" type="text" value={this.state.val} onChange={this.handleInputChange} />
          <button className="btn submit-btn" onClick={this.handleSubmit1}>submit</button>
        </div>
        <div className="todolist-lists">
          <ul className="todolist-list">
            <h4>Pending</h4>
            {pendingTasks.map((task) => <TodoItem key={task.id} task={task}
              onDelete={handelDel}
          ...
          <ul className="todolist-list">
            <h4>Completed</h4>
            {completedTasks.map((task) => <TodoItem key={task.id} task={task}
              onDelete={handelDel}
              onComplete={() => handleComplete(task)}
              onEdit={this.handelEdit}
              isEdit={task.id === this.state.isEdit}
